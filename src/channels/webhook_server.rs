...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Try to restart on an address already in use (the server's own address).
        // This reliably fails regardless of whether we are running as root.
        let invalid_addr = addr1;
        ...
        assert!(result.is_err(), "Bind to already-bound address should fail");
