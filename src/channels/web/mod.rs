...
impl Channel for GatewayChannel {
    ...
    async fn send_status(
        ...
        let event = match status {
            ...
            // Variants that map to their own SSE events or are not yet exposed
            StatusUpdate::JobStatus { job_id, status } => AppEvent::Status {
                message: format!("Job {job_id}: {status}"),
                thread_id,
            },
            StatusUpdate::JobResult { job_id, status } => AppEvent::Status {
                message: format!("Job {job_id} result: {status}"),
                thread_id,
            },
            StatusUpdate::RoutineUpdate { name, .. } => AppEvent::Status {
                message: format!("Routine updated: {name}"),
                thread_id,
            },
