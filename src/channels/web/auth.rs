...
fn check_email_domain(email: Option<&str>, allowed_domains: &[String]) -> Result<(), String> {
    ...
    let domain = email
        ...
        .ok_or_else(|| "Email address is missing a valid domain.".to_string())?;
    let allowed = allowed_domains.iter().any(|candidate| {
        let candidate = candidate.trim().to_ascii_lowercase();
        !candidate.is_empty() && candidate == domain
    });
    if allowed {
        Ok(())
    } else {
        Err("Login requires a verified email address from an authorized domain.".to_string())
    }
}
        ...
        match validate_oidc_jwt(oidc, jwt).await {
            Ok(sub) => {
                ...
                if !auth.oidc_allowed_domains.is_empty() {
                    ...
                    if let Err(msg) = check_email_domain(email.as_deref(), &auth.oidc_allowed_domains)
                    {
