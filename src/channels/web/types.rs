...
mod tests {
    ...
    #[test]
    fn test_routine_info_active_status() {
        let routine = make_routine_for_status_tests();

        let info = RoutineInfo::from_routine(&routine);
    ...
    #[test]
    fn test_routine_info_disabled_status() {
    ...
    #[test]
    fn test_routine_info_failing_status() {
        let mut routine = make_routine_for_status_tests();
        routine.consecutive_failures = 3;

        let info = RoutineInfo::from_routine(&routine);

        assert_eq!(info.status, "failing");
    }
}
